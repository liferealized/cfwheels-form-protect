var formprotect = formprotect || {};
(function(){

    formprotect.addListen = function(el,evt,func) {
    ...
    formprotect.getInputElementsByClassName = function (c) {
        var r = [];
        var j = 0;
    ...
    formprotect.logKeys = function() {
        //user hit a key, increment the counter
        keysPressed++;
        // Bas van der Graaf (bvdgraaf@e-dynamics.nl) 
        // Get all usedKeyboard inputs and set value
        var elKeyPressed = formprotect.getInputElementsByClassName('cfw-fp-kp');
    ...
    formprotect.addListen(document,'keypress', formprotect.logKeys);
    ...
    formprotect.timedMousePos = function() {
    ...
    var myInterval = window.setInterval(formprotect.timedMousePos,250)
    ...
    var calcDistance = function(aX,aY,bX,bY) {
        ...
        try {
            var elMouseMovement = formprotect.getInputElementsByClassName('cfw-fp-mm');
